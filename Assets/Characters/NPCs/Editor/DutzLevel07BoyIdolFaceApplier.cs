...
/// <summary>
/// Imports the compressed Boy Idol boss face photo and applies it on Level07.
/// </summary>
public static class DutzLevel07BoyIdolFaceApplier
{
    const string Level07Path = "Assets/Scenes/Dutz_Level07.unity";
    const string GiantName = "Boy Idol";
    const string TextureAssetPath = "Assets/Characters/Level07/Textures/BoyIdolBossFace.jpg";
    const string ResourcesPhotoPath = "Assets/Characters/NPCs/Resources/BoyIdolBossFacePhoto.jpg";
    const string MaterialPath = "Assets/Characters/NPCs/Resources/BoyIdolBossFace.mat";
    const int MaxEdge = 512;

    [MenuItem("Assets/Dutz Authoring/Apply Boy Idol Boss Face On Level07")]
    public static void ApplyFromMenu()
    {
        if (EditorApplication.isPlayingOrWillChangePlaymode)
        {
            Debug.LogWarning("[Dutz] Apply Boy Idol Boss Face requires Edit Mode.");
            return;
        }

        if (!ApplySilent(log: true))
            Debug.LogError("[Dutz] Failed to apply Boy Idol boss face.");
    }
    ...
    public static bool ApplySilent(bool log)
    {
        ...
        {
            Debug.LogError(
                "[Dutz] Missing BoyIdolBossFace.jpg — expected at Level07/Textures and NPCs/Resources.");
            return false;
        }
        ...
        if (texture == null)
        {
            Debug.LogError("[Dutz] Could not load BoyIdolBossFace texture after import.");
            return false;
        }
        ...
        if (giant == null)
            giant = DutzGiantBossNames.FindBoyIdol();
        if (giant == null)
        {
            Debug.LogError("[Dutz] Boy Idol not found in Level07.");
            return false;
        }
        ...
        if (log)
        {
            ...
            Debug.Log(
                $"[Dutz] Applied BoyIdolBossFace on Boy Idol ({MaxEdge}px max, {bytes} bytes JPG).");
        }
    ...
    static Material EnsureMaterial(Texture2D texture)
    {
        ...
        var template = AssetDatabase.LoadAssetAtPath<Material>(
            "Assets/Characters/NPCs/Resources/LironSintaBossFace.mat");
        if (template == null)
            template = AssetDatabase.LoadAssetAtPath<Material>(
                "Assets/Characters/NPCs/Resources/PiyayaBossFace.mat");
        if (template == null)
        {
            Debug.LogError("[Dutz] No boss-face material template found for Boy Idol.");
            return null;
        }

        var mat = new Material(template) { name = "BoyIdolBossFace" };
