...
/// <summary>
/// Imports the compressed K Bilyar boss face photo and applies it on Level07.
/// </summary>
public static class DutzLevel07KBilyarFaceApplier
{
    const string Level07Path = "Assets/Scenes/Dutz_Level07.unity";
    const string GiantName = "K Bilyar";
    const string TextureAssetPath = "Assets/Characters/Level07/Textures/KBilyarBossFace.jpg";
    const string ResourcesPhotoPath = "Assets/Characters/NPCs/Resources/KBilyarBossFacePhoto.jpg";
    const string MaterialPath = "Assets/Characters/NPCs/Resources/KBilyarBossFace.mat";
    const int MaxEdge = 512;

    [MenuItem("Assets/Dutz Authoring/Apply K Bilyar Boss Face On Level07")]
    public static void ApplyFromMenu()
    {
        if (EditorApplication.isPlayingOrWillChangePlaymode)
        {
            Debug.LogWarning("[Dutz] Apply K Bilyar Boss Face requires Edit Mode.");
            return;
        }

        if (!ApplySilent(log: true))
            Debug.LogError("[Dutz] Failed to apply K Bilyar boss face.");
    }
    ...
    public static bool ApplySilent(bool log)
    {
        ...
        {
            Debug.LogError(
                "[Dutz] Missing KBilyarBossFace.jpg — expected at Level07/Textures and NPCs/Resources.");
            return false;
        }
        ...
        if (texture == null)
        {
            Debug.LogError("[Dutz] Could not load KBilyarBossFace texture after import.");
            return false;
        }
        ...
        if (giant == null)
        {
            Debug.LogError("[Dutz] K Bilyar not found in Level07.");
            return false;
        }
        ...
        if (log)
        {
            ...
            Debug.Log(
                $"[Dutz] Applied KBilyarBossFace on K Bilyar ({MaxEdge}px max, {bytes} bytes JPG).");
        }
    ...
    static Material EnsureMaterial(Texture2D texture)
    {
        ...
        if (template == null)
        {
            Debug.LogError("[Dutz] No boss-face material template found for K Bilyar.");
            return null;
        }

        var mat = new Material(template) { name = "KBilyarBossFace" };
