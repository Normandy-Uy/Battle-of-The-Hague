...
/// <summary>
/// Imports the compressed MARKO LEKTA boss face photo and applies it on Level07.
/// </summary>
public static class DutzLevel07MarkoLektaFaceApplier
{
    const string Level07Path = "Assets/Scenes/Dutz_Level07.unity";
    const string GiantName = "MARKO LEKTA";
    const string TextureAssetPath = "Assets/Characters/Level07/Textures/MarkoLektaBossFace.jpg";
    const string ResourcesPhotoPath = "Assets/Characters/NPCs/Resources/MarkoLektaBossFacePhoto.jpg";
    const string MaterialPath = "Assets/Characters/NPCs/Resources/MarkoLektaBossFace.mat";
    const int MaxEdge = 512;

    [MenuItem("Assets/Dutz Authoring/Apply MARKO LEKTA Boss Face On Level07")]
    public static void ApplyFromMenu()
    {
        if (EditorApplication.isPlayingOrWillChangePlaymode)
        {
            Debug.LogWarning("[Dutz] Apply MARKO LEKTA Boss Face requires Edit Mode.");
            return;
        }

        if (!ApplySilent(log: true))
            Debug.LogError("[Dutz] Failed to apply MARKO LEKTA boss face.");
    }
    ...
    public static bool ApplySilent(bool log)
    {
        ...
        {
            Debug.LogError(
                "[Dutz] Missing MarkoLektaBossFace.jpg — expected at Level07/Textures and NPCs/Resources.");
            return false;
        }
        ...
        if (texture == null)
        {
            Debug.LogError("[Dutz] Could not load MarkoLektaBossFace texture after import.");
            return false;
        }
        ...
        if (giant == null)
            giant = DutzGiantBossNames.FindMarkoLekta();
        if (giant == null)
        {
            Debug.LogError("[Dutz] MARKO LEKTA not found in Level07.");
            return false;
        }
        ...
        if (log)
        {
            ...
            Debug.Log(
                $"[Dutz] Applied MarkoLektaBossFace on MARKO LEKTA ({MaxEdge}px max, {bytes} bytes JPG).");
        }
    ...
    static Material EnsureMaterial(Texture2D texture)
    {
        ...
        if (template == null)
        {
            Debug.LogError("[Dutz] No boss-face material template found for MARKO LEKTA.");
            return null;
        }

        var mat = new Material(template) { name = "MarkoLektaBossFace" };
