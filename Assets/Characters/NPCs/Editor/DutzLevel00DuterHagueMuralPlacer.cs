...
/// <summary>
/// Places an extra DUTERTEHAGUE center-road mural on Highway Straight 7 in Dutz_Level00.
/// </summary>
public static class DutzLevel00DuterHagueMuralPlacer
{
    const string Level00ScenePath = "Assets/Scenes/Dutz_Level00.unity";
    const string SegmentName = "Highway Straight 7";
    const string MuralObjectName = "DuterHagueMural_Highway Straight 7";
    ...
    public const string RootName = "DutzLevel00DuterHagueMural";

    /// <summary>Batch: -executeMethod DutzLevel00DuterHagueMuralPlacer.PlaceOnLevel00Batch</summary>
    ...
    public static bool PlaceOnLevel00(bool log)
    {
        ...
        if (!DutzLevel00DuterHagueMuralBuilder.SyncPhoto(log: false, force: false))
        {
            if (log)
                Debug.LogError("[Dutz] Could not sync DUTERTEHAGUE.png from public/DUTERTE MURALS/.");
            return false;
        }

        var texture = DutzLevel00DuterHagueMuralBuilder.LoadSyncedTexture();
        if (texture == null)
        {
            if (log)
                Debug.LogError("[Dutz] DUTERTEHAGUE texture missing after sync.");
            return false;
        }
        ...
        {
            if (log)
                Debug.LogError("[Dutz] Could not resolve Level 00 track reference for DUTERTEHAGUE mural.");
            return false;
        }
        ...
        {
            if (log)
                Debug.LogError("[Dutz] Highway Straight 7 not found for DUTERTEHAGUE mural.");
            return false;
        }

        ClearExistingMural();
        var root = new GameObject(RootName);
        Undo.RegisterCreatedObjectUndo(root, "Place DUTERTEHAGUE Mural");
        ...
        if (!DutzHighwayDeckSampler.TrySampleOnPath(path.Samples, PathPosition, out var centerSample))
        {
            ...
            if (log)
                Debug.LogError("[Dutz] Could not sample deck on Highway Straight 7.");
            return false;
        }
        ...
        {
            Debug.Log(
                $"[Dutz] Placed DUTERTEHAGUE cut-out mural (single panel) on {SegmentName}.");
        }
