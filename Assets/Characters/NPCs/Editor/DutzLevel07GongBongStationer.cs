...
/// <summary>
/// Stations Gong Bong at the end of Level07 Highway Bridge 5 — chase + burn + HP 50.
/// </summary>
public static class DutzLevel07GongBongStationer
{
    const string Level07Path = "Assets/Scenes/Dutz_Level07.unity";
    const string Bridge5Name = "Highway Bridge 5";
    const string GiantName = "Gong Bong";
    const int HitPoints = 50;
    const float EndAlongFraction = 0.92f;

    [MenuItem("Assets/Dutz Authoring/Station Gong Bong On Level07 Bridge5 End")]
    public static void StationFromMenu()
    {
        if (EditorApplication.isPlayingOrWillChangePlaymode)
        {
            Debug.LogWarning("[Dutz] Station Gong Bong On Level07 Bridge5 End requires Edit Mode.");
            return;
        }

        if (!StationSilent(log: true))
            Debug.LogError("[Dutz] Failed to station Gong Bong on Level07 Highway Bridge 5 end.");
    }
    ...
    public static bool StationSilent(bool log)
    {
        ...
        if (giant == null)
        {
            ...
            {
                if (transform != null && DutzGiantBossNames.IsGongBong(transform.name))
                {
                    giant = transform.gameObject;
                    break;
                }
            }
        }
        ...
        var bridge5 = GameObject.Find(Bridge5Name);
        if (bridge5 == null)
        {
            Debug.LogError($"[Dutz] '{Bridge5Name}' not found in Level07.");
            return false;
        }

        Undo.RecordObject(giant.transform, "Station Gong Bong On Bridge 5 End");
        ...
        var path = DutzHighwayDeckSampler.BuildSegmentPath(bridge5, Bridge5Name, spawn, travelForward);
        if (path.Samples == null || path.Samples.Count == 0)
        {
            Debug.LogError($"[Dutz] No deck samples for {Bridge5Name}.");
            return false;
        }

        if (!DutzHighwayDeckSampler.TrySampleOnPath(path.Samples, EndAlongFraction, out var sample))
        {
            Debug.LogError($"[Dutz] Could not sample end of {Bridge5Name}.");
            return false;
        }
        ...
        // Face incoming track traffic (toward Bridge 4 / earlier segments).
        ...
        {
            Undo.RecordObject(rb, "Station Gong Bong Rigidbody");
            rb.position = pivot;
        ...
        Undo.RecordObject(respawn, "Bake Gong Bong Bridge5 Spawn");
        ...
        {
            Debug.Log(
                $"[Dutz] Stationed {GiantName} at end of {Bridge5Name} at {pivot} " +
                $"(chase on, burn on, HP {HitPoints}, spawn baked).");
        }
    ...
    static void EnsureChaseHunter(GameObject go)
    {
        ...
        Undo.RecordObject(hunter, "Enable Gong Bong chase");
        hunter.enabled = true;
        EditorUtility.SetDirty(hunter);
    }
    ...
    static void EnsureBurn(GameObject go)
    {
        ...
        Undo.RecordObject(heat, "Enable Gong Bong burn");
